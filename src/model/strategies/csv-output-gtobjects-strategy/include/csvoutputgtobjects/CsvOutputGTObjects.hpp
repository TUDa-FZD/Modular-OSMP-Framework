...
namespace model {
    ...
    private:
        ...
        static void write_data_to_CSV(const std::string& path, double timestamp, uint64_t object_idx, float x, float y, float z, float roll, float pitch, float yaw, float width, float length, float height, bool is_moving);
    };
